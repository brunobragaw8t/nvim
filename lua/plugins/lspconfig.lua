  ...
  config = function()
    ...
    vim.api.nvim_create_autocmd("LspAttach", {
      ...
      callback = function(event)
        ...
        map("`", "<Cmd>lua vim.diagnostic.jump({ count = 1, float = true })<CR>", "Go to next diagnostic")
        map("~", "<Cmd>lua vim.diagnostic.jump({ count = -1, float = true })<CR>", "Go to previous diagnostic")
